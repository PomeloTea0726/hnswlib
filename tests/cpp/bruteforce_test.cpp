...
test_vs_recall(
    ...
    float query_per_second = 1e6 * qsize / total_time;

    cout << "qps: " << query_per_second << "\n";
    cout << "recall: " << recall << "\n";

    double flops_per_comp = 3.0 * 128.0;
    double total_flops = node_counter * flops_per_comp;
    double flops_per_cycle = total_flops / (total_time * 3.49 * 1e3);
    cout << "FLOPS per cycle: " << flops_per_cycle << "\n";
}
...
void sift_test1M() {
    ...
    snprintf(path_index, sizeof(path_index), "sift1m_bruteforce.bin");
    ...
    if (exists_test(path_index)) {
        ...
        appr_alg = new BruteforceSearch<int>(&l2space, path_index);
        cout << "Actual memory usage: " << getCurrentRSS() / 1000000 << " Mb \n";
    } else {
        cout << "Building index:\n";
        appr_alg = new BruteforceSearch<int>(&l2space, vecsize);
...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cout << "Usage: ./bruteforce_test <k> <dataset_path>" << std::endl;
        return 1;
    }

    ext_k = atoi(argv[1]);
    ext_dataset_path = argv[2];
